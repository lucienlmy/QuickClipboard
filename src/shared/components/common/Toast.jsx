...
function Toast({ id, message, type, position, size = TOAST_SIZES.MEDIUM, onClose }) {
  ...
  return (
    <div
      ...
      `}
    >
      <Icon size={sizeClasses.iconSize} className={`flex-shrink-0 ${iconClass}`} />
      <p className={`${sizeClasses.textSize} font-medium ${textClass} break-words`}>
        {message}
      </p>
      <button
        onClick={handleClose}
        className={`flex-shrink-0 ${sizeClasses.closeButtonPadding} rounded hover:bg-black/5 dark:hover:bg-white/5 transition-colors ${iconClass}`}
        aria-label="关闭"
      >
        <IconX size={sizeClasses.closeButtonSize} />
      </button>
    </div>
  )
}
