...
const ClipboardList = forwardRef(({ onScrollStateChange }, ref) => {
  ...
  const currentRangeRef = useRef({ startIndex: 0, endIndex: 0 })
  const snap = useSnapshot(navigationStore)
  const clipSnap = useSnapshot(clipboardStore)
  const settings = useSnapshot(settingsStore)

  const itemsArray = useMemo(() => {
    return Array.from({ length: clipSnap.totalCount }, (_, i) => clipSnap.items.get(i) || null)
  ...
  const getDefaultItemHeight = () => ({ auto: 90, large: 120, medium: 90, small: 50 }[settings.rowHeight] ?? 90)
        ...
        <SortableContext items={itemsWithId.map(item => item._sortId)} strategy={strategy}>
          <Virtuoso
            ...
            itemContent={(index) => {
              ...
              const skeletonHeight = { auto: 'min-h-12 h-20', small: 'h-12', medium: 'h-20', large: 'h-32' }[settings.rowHeight] ?? 'h-20'
              ...
              return (
                <div className="px-2.5 pb-2 pt-1 relative">
                  <div
                    className={`${isAutoHeight ? 'absolute inset-0' : ''} rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 ${skeletonHeight} animate-pulse`}
                    style={{ animation: `pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite, fadeOut 0.3s ease-out ${animationDelay + 200}ms forwards` }}
                  >
                    <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-3/4 mb-2"></div>
                    <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded w-1/2"></div>
                  </div>
            ...
            isScrolling={(scrolling) => scrolling ? handleScrollStart() : handleScrollEnd()}
            style={{ height: '100%' }}
          />
        </SortableContext>
      </div>
