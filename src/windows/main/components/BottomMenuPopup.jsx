...
import { IconCheck, IconPin, IconPinned, IconChevronRight } from '@tabler/icons-react'
  ...
  return (
    <>
      ...
      >
        ...
        {isOpen && (
          <div 
            className={`groups-panel absolute bottom-full right-0 backdrop-blur-xl bg-white/95 dark:bg-gray-800/95 border border-b-0 border-gray-200/50 dark:border-gray-700/50 rounded-t-xl shadow-2xl z-40 overflow-hidden flex flex-col ${
              isClosing ? 'animate-slide-down' : 'animate-slide-up'
            }`}
            style={{ width: `${width}px`, maxHeight: '350px' }}
          >
            {/* 头部 */}
            ...
            <div className="flex-1 overflow-y-auto py-1">
              {menuItems.map(menuItem => {
                ...
                return (
                  <div key={menuItem.id} className="border-b border-gray-200/30 dark:border-gray-700/30 last:border-b-0">
                    <div
                      onClick={() => toggleMenuItem(menuItem.id)}
                      className="flex items-center gap-2 px-2.5 py-1.5 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-all"
                      title={menuItem.label}
                    >
                      {menuItem.icon && (
                        <div className="flex-shrink-0">
                          <menuItem.icon size={14} className="text-gray-600 dark:text-gray-400" />
                        </div>
                      )}
        ...
        <button
          onClick={togglePopup}
          className={`flex items-center justify-center gap-1.5 h-5 transition-all duration-300 ${
            isOpen
        ...
        >
          {Icon && <Icon size={12} />}
          <span className="text-[10px] font-medium truncate" style={{ maxWidth: `${width - 40}px` }}>
            {label}
          </span>
        </button>
      </div>
