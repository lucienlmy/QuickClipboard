...
**QuickClipboard**는 Tauri 2 + Rust + React로 구축된 크로스 플랫폼 클립보드 강화 도구입니다 (현재 Windows 및 Android 지원). 복사하는 순간 자동으로 텍스트, 이미지, 리치 텍스트, 파일을 기록하여 이전에 복사한 모든 내용을 언제든지 찾아볼 수 있습니다. 기록뿐만 아니라 스크린샷, 이미지 고정, OCR, LAN 동기화 등 다양한 기능을 통합하여 일상 업무 생산성을 획기적으로 향상시킵니다.
...
| LAN 동기화               | 클립보드 동기화 (텍스트 / 이미지) · 서버 / 클라이언트 모드 · 페어링 코드 / QR 코드 연결 · 수신 후 클립보드에 자동 쓰기 · 자동 재연결 |
| LAN 채팅 및 전송         | LAN 채팅 · 파일 / 폴더 전송 · 온라인 장치 목록 · 수신 확인 · 전송 진행 알림                                             |
