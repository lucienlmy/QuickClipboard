...
use super::webdav_client::{WebdavClient, WebdavUploadProgressCallback};
...
pub async fn upload_file_with_progress(
    ...
    let upload_progress = progress.map(|callback| {
        ...
        }) as WebdavUploadProgressCallback
    });

    if let Err(error) = client
        .put_file_with_progress(&format!("{}/data", object_dir), &source, upload_progress)
        .await
    {
        let _ = delete_cloud_object(client, &object_dir).await;
        return Err(error);
    }
...
pub async fn delete_file(client: &WebdavClient, file_id: &str) -> Result<(), String> {
    ...
    delete_cloud_object(client, &object_dir).await?;
...
async fn delete_cloud_object(client: &WebdavClient, object_dir: &str) -> Result<(), String> {
